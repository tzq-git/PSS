...
<nav class="breadcrumb">
    ...
    维护基本资料 <span class="c-gray en">&gt;</span> 销售员资料维护 <a
...
<div class="page-container">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        <span class="l">
            ...
            <a href="javascript:;"
               onclick="item_add('添加','SalePerson?oper=insert','800','500')"
    ...
    <div class="mt-20">
        <table id="datalist"  class="table table-border table-bordered  table-bg ">
            ...
            <c:forEach var="item" items="${DataList }">
                <tr class="text-c">
                    <td><input type="checkbox" value="${item.sid }" name=""></td>
                    <td>${item.sid }</td>
                    <td>${item.sCName }</td>
                    <td>${item.sEName }</td>
                    <td>${item.phone }</td>
                    ...
                    <td>
                        <a title="删除" href="javascript:;"
                           onclick="item_del(this,${item.sid})" class="ml-5"
                           style="text-decoration: none"><i class="Hui-iconfont">&#xe6e2;</i>
                        </a>
                        <a title="编辑" href="javascript:;"
                           onclick="item_edit('编辑信息','SalePerson?oper=update&id=${item.sid}','600','600','500')"
                           class="ml-5" style="text-decoration: none"> <i
                                class="Hui-iconfont">&#xe6df;</i></a>
                        <a title="查看"
                           href="javascript:;"
                           onclick="item_detail('查看具体信息','SalePerson?oper=detail&id=${item.sid }','600','600','500')"
                           class="ml-5" style="text-decoration: none"> <i
                                class="Hui-iconfont">&#xe707;</i></a>
                    </td>
                </tr>
...
<script type="text/javascript">
    ...
    $(function(){
        $("#clearSearch").click(function(){
            location.href = "SalePerson?oper=list";
        });
    });
    ...
    function item_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.ajax({
                type: 'POST',
                url: 'SalePerson?oper=deleteDeal&id=' + id,
            //    dataType: 'json',
                success: function(data){
                    if(data=="ok"){
                        ...
                        layer.msg('请去客户管理处指派其他销售员接手客户', {
                            icon :1,
                            tiem :1000
                        });
                    }
                    else{
                        layer.msg('该销售员与公司有交易记录，不允许被删除', {
                            icon :1,
                            tiem :2000
                        });
                    }

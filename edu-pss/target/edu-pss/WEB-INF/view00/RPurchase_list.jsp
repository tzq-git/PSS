...
<nav class="breadcrumb">
    ...
    采购维护 <span class="c-gray en">&gt;</span> 退货管理 <a
...
<div class="page-container">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        <span class="l">
            ...
            <a href="javascript:;"
               onclick="item_add('添加','RPurchase?oper=insert','800','500')"
    ...
    <div class="mt-20">
        <table id="datalist"  class="table table-border table-bordered  table-bg ">
            ...
            <c:forEach var="item" items="${DataList }">
                <tr class="text-c">
                    <td><input type="checkbox" value="${item.rPurchaseId }" name=""></td>
                    <td>${item.rPurchaseId }</td>
                    <td>${item.pName }</td>
                    <td>${item.outDate }</td>
                    <td>${item.gName }</td>
                    <td>${item.price }</td>
                    ...
                    <td>
                        <a title="删除" href="javascript:;"
                           onclick="item_del(this,${item.rPurchaseId})" class="ml-5"
                           style="text-decoration: none"><i class="Hui-iconfont">&#xe6e2;</i>
                        </a>
                        <a title="编辑" href="javascript:;"
                           onclick="item_edit('编辑信息','RPurchase?oper=update&id=${item.rPurchaseId}','600','600','500')"
                           class="ml-5" style="text-decoration: none"> <i
                                class="Hui-iconfont">&#xe6df;</i></a>
                        <a title="查看"
                           href="javascript:;"
                           onclick="item_detail('查看具体信息','RPurchase?oper=detail&id=${item.rPurchaseId }','600','600','500')"
                           class="ml-5" style="text-decoration: none"> <i
                                class="Hui-iconfont">&#xe707;</i></a>
                    </td>
                </tr>
...
<script type="text/javascript">
    ...
    $(function(){
        $("#clearSearch").click(function(){
            location.href = "RPurchase?oper=list";
        });
    });
    ...
    /*用户-查看*/
    function rPurchase_show(title,url,id,w,h){
        layer_show(title,url,w,h);
    }
    /*用户-停用*/
    function rPurchase_stop(obj,id){
        layer.confirm('确认要停用吗？',function(index){
            $.ajax({
                ...
                success: function(data){
                    $(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="rPurchase_start(this,id)" href="javascript:;" title="启用"><i class="Hui-iconfont">&#xe6e1;</i></a>');
    ...
    /*用户-启用*/
    function rPurchase_start(obj,id){
        layer.confirm('确认要启用吗？',function(index){
            $.ajax({
                ...
                dataType: 'json',
                success: function(data){
                    $(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="rPurchase_stop(this,id)" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a>');
    ...
    /*项目-编辑*/
    function rPurchase_edit(title,url,id,w,h){
        layer_show(title,url,w,h);
    }
    /*密码-修改*/
    ...
    function item_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.ajax({
                type: 'POST',
                url: 'RPurchase?oper=deleteDeal&id=' + id,
            //    dataType: 'json',
                success: function(data){
                    if(data=="ok"){

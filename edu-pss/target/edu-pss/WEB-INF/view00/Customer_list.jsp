...
<nav class="breadcrumb">
    ...
    维护基本资料 <span class="c-gray en">&gt;</span> 客户资料维护 <a
...
<div class="page-container">
    <div class="text-c">
        <form action="Customer" method="get">
            ...
            <input type="text" class="input-text" value="${searchName }"
                   style="width: 250px" placeholder="客户名称"
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        <span class="l">
            ...
            <a href="javascript:;"
               onclick="item_add('添加','Customer?oper=insert','800','500')"
    ...
    <div class="mt-20">
        <table id="datalist"  class="table table-border table-bordered  table-bg ">
            ...
            <tr class="text-c">
                ...
                <th width="40">ID</th>
                <th width="100">客户名</th>
                <th width="100">所属公司简称</th>
                <th width="100">所属公司全称</th>
            ...
            <c:forEach var="item" items="${DataList }">
                <tr class="text-c">
                    <td><input type="checkbox" value="${item.cid }" name=""></td>
                    <td>${item.cid }</td>
                    <td>${item.cName }</td>
                    <td>${item.companySN }</td>
                    <td>${item.companyAN }</td>
                    <td>${item.sCName }</td>
                    <td>${item.phone }</td>
                    <td>${item.fax }</td>
                    <td>${item.address }</td>
                    ...
                    <td>
                        <a title="删除" href="javascript:;"
                           onclick="item_del(this,${item.cid})" class="ml-5"
                           style="text-decoration: none"><i class="Hui-iconfont">&#xe6e2;</i>
                        </a>
                        <a title="编辑" href="javascript:;"
                           onclick="item_edit('编辑信息','Customer?oper=update&id=${item.cid}','600','600','500')"
                           class="ml-5" style="text-decoration: none"> <i
                                class="Hui-iconfont">&#xe6df;</i></a>
                        <a title="查看交易记录"
                           href="javascript:;"
                           onclick="item_detail('客户交易记录]','DealRecord?oper=detail&id=${item.cid }','800','600','500')"
                           class="ml-5" style="text-decoration: none"> <i
                                class="Hui-iconfont">&#xe707;</i></a>
                    </td>
                </tr>
...
<script type="text/javascript">
    ...
    $(function(){
        $("#clearSearch").click(function(){
            location.href = "Customer?oper=list";
        });
    });
        ...
        layer.confirm('确认要停用吗？',function(index){
            $.ajax({
                ...
                success: function(data){
                    $(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="customer_start(this,id)" href="javascript:;" title="启用"><i class="Hui-iconfont">&#xe6e1;</i></a>');
    ...
    function customer_start(obj,id){
        layer.confirm('确认要启用吗？',function(index){
            $.ajax({
                ...
                dataType: 'json',
                success: function(data){
                    $(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="customer_stop(this,id)" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a>');
    ...
    function item_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.ajax({
                type: 'POST',
                url: 'Customer?oper=deleteDeal&id=' + id,
            //    dataType: 'json',
                success: function(data){
                    if(data=="nodelete"){
                        layer.msg('有交易记录的客户不能删除！',{
                            icon:1,
                            time:1000
                        });
                    }else if(data=="ok"){
